...
原始 PSD → 最近16帧平均谱/最大谱 → 32768点切窗（步长16384）
         → min/max归一化 → ISA TensorRT SCN → 解码/NMS
         → CNR候选池 → 频谱占用证据/信道级聚合与确认
         → 信道跟踪/边界稳定/完整频段重测 → 白名单与告警/UI结果
```

从首帧开始渐进检测，满 16 帧后保持滑动窗口。默认置信度 0.4、NMS IoU 0.5、
CNR 3 dB；窗口尾部按最低值补齐，仅有效原始区域参与电平计算。宽带信道聚合默认启用，
从原始 PSD 估计噪底和占用，最多维护 16 帧／2 秒证据，至少 3 次确认新归并、5 次确认拆分；
持续低噪底空洞不会被跨越，缺失信号只有连续可靠缺失达到次数和 0.5 秒数据时间后才移除。
SCN 候选先保留未过 CNR 的聚合候选，只有组成有效宽带区域后才按全信道重测 CNR；未聚合候选保持原筛选行为。
...
跟踪使用频段重叠、带宽比和中心距离共同约束的一对一关联。信道聚合以父信道维持身份，保留贡献候选及待观测状态；未知结果不增加缺失计数或告警计时。可在“系统设置 → SCN 检测 → 宽带信道聚合”关闭聚合做 A/B 对照或配置可选信道先验。DetectionLab JSONL 可检查候选、贡献引用、占用掩码摘要及状态诊断，`--dump` 导出详细噪底和占用掩码。

原始单信号跟踪使用频段重叠、带宽比和中心距离共同约束的一对一关联。默认用最近 5 次已接受边界的中位数
