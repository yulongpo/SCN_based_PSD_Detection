 ...
 */
class AlertRoleDialog : public QDialog
{
    Q_OBJECT
...
public:
     ...
     * @param parent 父控件
     */
    explicit AlertRoleDialog(QWidget *parent = nullptr);
     ...
     * @param freqEnd  截止频率
     * @param sigType  信号类型
     * @param lvlMode  告警等级模式（2=一般，3=严重）
     * @param bwMax    信号最大带宽
     * @param bwMin    信号最小带宽
     * @param remark   备注
     */
    ...
    /**
     * @brief 获取告警等级模式值（2=一般，3=严重）
     * @return 告警等级模式
     */
    int alertLevelMode() const;
...
private:
    ...
    HQCComboBox *m_sigTypeCbx  = nullptr;  ///< 信号类型下拉框
    HQCComboBox *m_alertLvlCbx = nullptr;  ///< 告警等级下拉框
    HQLineEdit  *m_bwMaxEdit   = nullptr;  ///< 信号最大带宽输入框
    HQLineEdit  *m_bwMinEdit   = nullptr;  ///< 信号最小带宽输入框
