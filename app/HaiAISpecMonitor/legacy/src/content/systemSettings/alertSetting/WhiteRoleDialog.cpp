...
WhiteRoleDialog::WhiteRoleDialog(QWidget *parent)
    : QDialog(parent)
{
    setObjectName("WhiteRoleDialog");
    ...
    // 固定弹窗尺寸
    setFixedSize(scaledPx(740, 740), scaledPx(350, 350));
    ...
    connect(&ThemeManager::instance(), &ThemeManager::themeChanged,
            this, &WhiteRoleDialog::applyTheme);
    applyTheme();
...
void WhiteRoleDialog::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
    QPainter p(this);
...
void WhiteRoleDialog::setupTitleBar()
{
    const int titleH = scaledPx(44, 30);
    const int btnSz  = scaledPx(21, 14);

    m_titleBar = new QWidget(this);
    m_titleBar->setObjectName("WhiteRoleDialogTitleBar");
    ...
    m_accentLine->setObjectName("WhiteRoleDialogAccentLine");
    ...
    m_titleLabel->setObjectName("WhiteRoleDialogTitle");
    ...
    m_closeBtn->setObjectName("WhiteRoleDialogCloseBtn");
...
void WhiteRoleDialog::setupForm()
{
...
void WhiteRoleDialog::setupButtons()
{
...
void WhiteRoleDialog::applyTheme()
{
    const auto &tm = ThemeManager::instance();

    if (m_titleBar)
        m_titleBar->setStyleSheet("#WhiteRoleDialogTitleBar { background-color: rgb(14,14,29); }");
...
bool WhiteRoleDialog::eventFilter(QObject *watched, QEvent *event)
{
    if (!isVisible())
...
QString WhiteRoleDialog::ruleName() const
{
    return m_nameEdit ? m_nameEdit->text().trimmed() : QString();
}

QString WhiteRoleDialog::freqStart() const
{
    return m_freqStart ? m_freqStart->text().trimmed() : QString();
}

QString WhiteRoleDialog::freqEnd() const
{
    return m_freqEnd ? m_freqEnd->text().trimmed() : QString();
}

QString WhiteRoleDialog::remark() const
{
    return m_remarkEdit ? m_remarkEdit->text().trimmed() : QString();
}

void WhiteRoleDialog::setDialogTitle(const QString &title)
{
    if (m_titleLabel)
        m_titleLabel->setText(title);
}

void WhiteRoleDialog::setEditData(const QString &name, const QString &freqStart,
...
int WhiteRoleDialog::scaledPx(int designPx, int min) const
{
