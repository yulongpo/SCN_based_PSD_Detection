...
本说明记录 SCN 后端的独立数据与部署契约。系统设置现同时提供 FFSCN 17 阶后端；
其时间窗、短谱插值、ONNX/TensorRT 转换和对照验收见
[FFSCN 后端接入方案](ffscn_backend_plan.md)。
...
- SCN 输出以热图三点局部极大值、TopK=200、置信度 >=0.1、stride=4 解码。
...
配置为强类型 `DetectionConfig`。相同配置不变更状态；窗口长度或跟踪稳定设置变更重置累积及跟踪；
