...
- 对信道聚合保留独立的未按 CNR 淘汰候选池；片段聚合后在完整信道频段重测，再按同一 CNR 阈值筛选。
  未能聚合的候选仍沿用原有逐片段筛选结果。`channelCandidates[].passedCnr` 保留候选进入聚合前的筛选状态。
...
跟踪常规关联同时要求交集/较窄带宽不低于 0.45、带宽比不高于 2、中心距离不高于
`max(轨迹带宽*0.25, 2*binWidth)`；候选按 IoU、中心距离、带宽比、轨迹 ID 和观测索引
确定性排序，并执行一对一匹配。密集匹配按每个观测最多 64 条边缓存，缓存耗尽时重新扫描
尚未匹配的轨迹，不截断合法匹配边；突变唯一性检查只计数、不存储完整匹配矩阵。

默认启用边界稳定：每条轨迹保留最近 5 次已接受观测，分别取中心和带宽中位数，再以
`alpha=0.35` 的 EMA 更新；对外区间按采集网格向外对齐并裁剪。首次观测立即输出。
若边缘变化超过 `max(0.15*轨迹带宽, 2*binWidth)` 或带宽比超过 2，只有旧轨迹与观测
相互唯一时才进入突变候选；稳定边界先保持，连续 3 个不同帧号的一致候选后切换。
高级容差默认候选中心差不超过 `max(候选中位带宽*0.10, 2*binWidth)`，带宽比不超过 1.2。
歧义候选不强制沿用旧 ID；结果缺失或帧序号不连续会中断确认。未匹配轨迹保留 1 秒供
后续关联，但不会作为当前信号显示。

`DetectionResult::detections` 仍保留融合后的原始边界和测量值；`trackedDetections` 通过相同
ID 提供稳定边界、重测值、状态及关联诊断。稳定边界测量在完整的 16 帧平均谱/最大谱上重算，
沿用中间约 60% 信号区和两侧约 10% 噪声区；Both 分支整组选择信号电平较高者，平手选平均谱。
白名单、告警、信号表、标记和回放结果均使用稳定业务结果；原始结果仍可用于详情和 DetectionLab
对照。关闭“边界稳定”时保留原始边界和检测测量，用于 A/B 对照。
更新在工作副本上完成，取消或异常不提交跟踪、稳定状态或累积帧。
次数指独立检测观测命中次数，不是采集帧总数，也不是 UI 刷新次数。
新监测、源/频率网格/参考电平变化、回放跳转和循环重启时清空历史与 ID，并递增轨迹分段；
应用层据此结束旧业务分段，防止重用的跟踪 ID 串接告警历史。
无效帧不进入累积，取消/异常推理回滚当前帧的累积变更。

## 宽带信道聚合

默认启用信道级聚合，以减少 Wi-Fi、蜂窝等宽带占用被 SCN 拆为多个碎片后的结果抖动；此逻辑只根据 PSD 证据归并占用信道，不能据此推断协议或区分同频发射机。关闭后可与原始 SCN 结果做 A/B 对照。

聚合证据直接来自原始 PSD：分析单元宽度为 `max(RBW, 4*binWidth, 25 kHz)`，单元功率取中位数；从不包含候选的局部窗口估计 P20 噪底并插值。高于噪底 6 dB 的单元建立占用区，3 dB 门限延伸边缘；无可靠噪底的区域为未知，不能单独支撑归并。最多保留 16 个有效处理帧且跨度不超过 2 秒的证据。

自动归并需要多个候选、至少 1 MHz（或 16 个分析单元）宽、历史占用覆盖率至少 70%、区域总空洞不超过 20%，且至少 2 个历史帧在频段左右两侧都有占用。默认最大宽度 200 MHz；短间隙最多为 `min(1 MHz, 频段宽度*5%)`，连续可靠的低噪底间隔禁止跨越。新关系需连续 3 次有效结果确认，拆分需 5 次确认。先验仅限制/辅助归属，不单独触发检测、不吸附标称边界；首次运行不配置先验。孤立窄带/短突发仍按原通路即时输出，不因聚合等待。

确认的聚合信道保留父 ID 和稳定外边界；内部碎片数变化不改变身份。可靠缺失需连续 3 次且累计有效数据时间至少 0.5 秒后才确认消失；未知观测不计作缺失，结果保留为“暂时未观测”，告警计时冻结。聚合后的信号电平、噪声和 CNR 在完整信道范围重测；无有效噪声样本会标为测量无效，依赖 CNR 的告警条件按未知处理。

`DetectionResult` 同时保留原始 SCN 检测、未筛 CNR 的聚合候选、信道输出及贡献索引，不假设候选与最终信道一对一。贡献引用由序列号和候选索引组成。DetectionLab 的 JSONL 输出 `channelCandidates`、`channelDetections`、`channelGroupingDiagnostics`（候选组/相邻连接的频段、当前占用指标、结果 ID 及具体通过/拒绝原因）、占用证据摘要及归并/待确认/缺失诊断；开启 `--dump` 时另写 `_channel_evidence.json`（噪底、known 与 occupancy 掩码）。CSV 保留原始观测与边界稳定结果；完整贡献关系以 JSONL 为准。
...
配置为强类型 `DetectionConfig`。相同配置不变更状态；任何影响候选、融合、累积、信道聚合或跟踪的运行时参数变更均重置相关状态并开始新的业务检测分段；
检测配置版本在下一结果生效时结束旧业务分段，旧告警按“检测配置变化”结束；更换 engine/GPU
或切换检测开关必须停止后重新初始化。
...
匹配发生在稳定频段上，告警的频率和带宽使用白名单配置区间，电平、CNR 和置信度使用代表稳定结果；
原始检测边界和稳定重测信息同时写入业务详情与告警历史。
